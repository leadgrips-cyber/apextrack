...
import { listAdminOffers } from "../../services/offers";
...
export function AffiliatePostbacksView() {
  ...
  function openEdit(row: AdminPostbackRow) {
    ...
    setForm({
      ...
      offer_id:     row.offer_id != null ? String(row.offer_id) : "",
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black theme-text-main tracking-tight">Affiliate Postbacks</h1>
          <p className="text-sm theme-text-muted mt-1">Manage outbound publisher postback URLs.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm theme-text-muted">{total} postback{total !== 1 ? "s" : ""}</span>
      </div>
          ...
          <table className="w-full min-w-[900px] text-sm">
            ...
            <tbody className="divide-y theme-divide">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-8 text-center theme-text-muted text-sm">No postbacks found.</td></tr>
              ) : rows.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition">
                  ...
                  <td className="px-4 py-3 text-xs theme-text-secondary">
                    {row.offer_name ?? <span className="theme-text-muted italic">Global</span>}
                  </td>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border theme-border shadow-2xl w-full max-w-lg p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold theme-text-muted uppercase tracking-wider mb-1">Offer (optional — blank = global)</label>
              <select
                value={form.offer_id}
                onChange={(e) => setForm((f) => ({ ...f, offer_id: e.target.value }))}
                className="w-full rounded-xl border theme-border bg-white dark:bg-slate-800 theme-text-main text-sm px-3 py-2"
              >
                <option value="">Global (all offers)</option>
                {offers.map((o) => (
